...
GMathVec2<Scalar> const CDF1Sampler<Scalar>::max_domain(-std::numeric_limits<Scalar>::max(),
...
template<typename Scalar>
Scalar CDF1SamplerUniform<Scalar>::sample(Scalar const sx, Scalar& pdf, Domain const& domain) const
{
    assert(0. <= sx && sx <= 1.);
                         ...
                         // This means we are sampling uniformly between domain_min and domain_max.
    {
        ...
        return linear_interpolation(domain_min, domain_max, sx);
    }
...
template<typename Scalar>
Scalar CDF1SamplerNonUniform<Scalar>::sample(Scalar const sx, Scalar& pdf,
        GMathVec2<Scalar> const& domain) const
{
    assert(0. <= sx && sx <= 1.);
