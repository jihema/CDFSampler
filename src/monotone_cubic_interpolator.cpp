...
MonotoneCubicInterpolator<Scalar>::MonotoneCubicInterpolator(const vector<Scalar> & x,
...
Scalar MonotoneCubicInterpolator<Scalar>::evaluate(Scalar x) const
{
    ...
    return cubic_interpolation(m_f[x_idx], m_f[x_idx + 1], m_fp[x_idx], m_fp[x_idx + 1],
            x_it.m_theta);
}
